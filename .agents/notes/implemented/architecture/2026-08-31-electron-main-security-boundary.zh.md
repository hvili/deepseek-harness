...
Preload 为兼容 Electron sandbox 使用 CommonJS，仅暴露有类型的 `desktopBridge`。Main 在把逻辑 IPC 请求转成等价 loopback gateway 请求前验证输入，跟踪取消和订阅，拥有标题栏动作，并过滤通知意图。不存在托盘或更新通道。Web 与 desktop 层把共享锁路径写成 `!!js dshHomePath('interactive-host')`，从而保留 Loader 表达式语义，而不依赖严格 YAML 无法解析的反引号 scalar。增强插件使用的上游 `cordis` peer 会作为 `@deepseek-ai/cordis` 的 workspace alias 安装；打包后的别名是 ESM 转发 shim，因此两个名称暴露 Host 的同一组框架对象，不会分裂 Service 身份。electron-builder 生成未签名的 NSIS 与 ZIP x64 产物，卸载时不清除数据；它保持 `resources/app` 未封装，使受管 Profile 的 junction 能解析到真实包目录，并保留 node-pty 的 Windows x64 N-API 预编译产物而不在打包中重编 native modules。桌面 manifest 会在打包前显式闭合所有可达 workspace peer 和本地增强 Bundle 根，因此安装后的 Host 不依赖仅在开发环境存在的链接。`afterPack` hook 从 Koffi、Sharp、Codex 和 ripgrep 父包的精确 `optionalDependencies` 解析 Windows x64 包，验证包身份、版本、OS 和 CPU，并把完整包复制到各运行时 loader 请求的别名下。分发只有在打包验证通过后，才为安装器与 ZIP 原子写入 `SHA256SUMS.txt`。
