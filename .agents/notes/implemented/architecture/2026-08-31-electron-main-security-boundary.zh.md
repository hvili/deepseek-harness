...
Preload 为兼容 Electron sandbox 使用 CommonJS，仅暴露有类型的 `desktopBridge`。Main 在把逻辑 IPC 请求转成等价 loopback gateway 请求前验证输入，跟踪取消和订阅，拥有标题栏动作，并过滤通知意图。不存在托盘或更新通道。声明 NSIS 与便携 x64 目标，卸载时不清除数据。
