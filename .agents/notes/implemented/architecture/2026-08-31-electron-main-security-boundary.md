...
Preload is CommonJS for Electron sandbox compatibility and exposes only the typed `desktopBridge`. Main validates logical IPC requests before translating them to loopback-equivalent gateway requests, tracks cancellation and subscriptions, owns caption actions, and filters notification intents. It has no tray or update channel. NSIS and portable x64 targets are declared without an uninstall data purge.
