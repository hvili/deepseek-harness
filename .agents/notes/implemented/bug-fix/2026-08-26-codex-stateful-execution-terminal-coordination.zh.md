...
- **只等待 `turn/completed`**：否决，因为 child 退出、输入关闭、传输错误或畸形终态通知都可能在没有该通知的情况下发生。
- **让 `turn/interrupt` 成为取消的权威结果**：否决，因为失败或没有响应的 interrupt 不能阻止本地取消保持有界结束。
- **公开原始进程或上游错误**：否决，因为路径、命令、stderr、凭据和上游文本都不是稳定或安全的公开诊断信息。

## 后果

本地取消会立即 settle，清理则继续走常规释放阶梯；尽力式 interrupt 可能失败，但不会改变有界结果。每条终态路径都会观察 child 与传输失败，关闭 wire，并等待完整进程树停稳。这会给底层 stateful API 增加显式协调与固定安全错误，但一次性 provider 及所有 DeepSeek 原生界面保持不变。
