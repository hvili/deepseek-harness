...
[`core-patches.json`](core-patches.json) 包含初始下游范围内的每个非合并提交。每项记录受影响区域、一种处置分类、验证方式、上游状态和删除条件。允许的分类为：
...
新的运行时或构建补丁进入 `product/main` 后，必须由后续审计提交加入清单或替换已有条目。仅含治理元数据的改动由 `product:verify` 直接验证。每次官方同步至少要删除、合并或重新证明一个保留条目。
