...
function fixture(): string {
  // realpathSync(tmpdir()) matters on macOS: os.tmpdir() reports /var/folders/…
  // while vite/rollup resolve through the /private/var symlink, and the
  // mismatched spellings make the vite:build-html plugin emit an HTML asset
  // whose fileName is a ../-heavy relative path rollup then rejects.
  const root = mkdtempSync(join(realpathSync(tmpdir()), 'dsh-browser-notices-'))
  roots.push(root)
