...
- **Writes hold a cross-process writer lock.** The read-render-rename cycle runs under a `wx`-created `<file>.lock` sibling with exponential backoff and a 2 s acquisition deadline. The provider lands the raw document, revision, and resolved mirror immediately after the rename, then releases the lock before dispatching settings listeners; a slow synchronous listener therefore cannot make another process exhaust the acquisition window. A contender times out without removing an existing lock because age cannot distinguish a crashed owner from a paused live writer; orphan recovery is an operator action. Readers never take the lock: the rename commit is atomic, so reloads are always consistent.
...
- **Reloads and writes share one operation chain.** Watcher refreshes and persists from every namespace queue run one at a time in queue order; each render sees the text the previous operation committed. Post-lock listener dispatch remains inside this chain, preserving notification order without extending cross-process lock ownership.
