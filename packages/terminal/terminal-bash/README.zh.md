...
pwsh 设置命令会反复重新提交，直到观察到提示符标记。在 pwsh 交互循环启动之前提交的一行命令可能被 shell 自身的启动终端探测消费掉——只产生回显而不执行，因此一次已结算的发送并不证明设置已运行。未获确认的结算会重新提交这个幂等的设置；从第三次尝试起，先提交一个空行取消被部分消费的行。确认标记之后，启动会等待输出流归于静默——重提交队列或终端自身的滞后回显否则会让下一条发送按残留的启动输出结算。最近一次非空的有界启动输出会保留为启动消息，提示符标记仍然是就绪条件。
