/**
 * @deepseek-ai/dsh-host-desktop-carrier — the Electron desktop carrier: a
 ...
 * the desktop-app bundle swaps the `webserver` row for this package, and the
...
export class DesktopWebServer extends Service {
  ...
  register(route: WebRoute): () => void {
    ...
    if (table.has(route.path)) {
      throw new Error(`desktop-carrier: duplicate ${route.kind} route "${route.path}"`)
    }
  ...
  registerUpgrade(route: WebUpgradeRoute): () => void {
    if (this.upgrades.has(route.path)) {
      throw new Error(`desktop-carrier: duplicate upgrade route "${route.path}"`)
    }
  ...
  registerFallback(handler: WebRoute['handler']): () => void {
    if (this.fallback !== undefined) {
      throw new Error('desktop-carrier: fallback already registered')
    }
