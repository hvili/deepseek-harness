...
The desktop shell's `desktop-app` bundle swaps the web carrier's `webserver` row for this package, and the app's protocol registration calls `dispatch` per request. This package never prints; it has no URL line because the shell has no URL.
