...
## Persistent-thread foundation and explicit stateful seam

`thread-state.ts` provides the narrow state model for an explicit stateful Codex consumer. It creates only `ephemeral: false` threads, validates their opaque id as `CodexThreadId`, stores exactly one required `codex/thread-reference` event in the DSH-owned append-only Session log, and reopens that exact id with `thread/resume` after a fresh persistence mount. A resumed response must still describe the same non-ephemeral thread; duplicate, malformed, unknown-field, or unsupported references fail closed. This record is an external execution reference only: DSH does not copy the Codex project database, history, or settings.

[`DshSessionCodexThreadJournal`](src/session-journal.ts) is the production DSH Session adapter. The owning product first loads or prepares a complete, validated Session through its selected [`SessionPersistence`](../../session/session-persistence/README.md) backend, then constructs the journal with that Session and `ctx.sessions`. `load()` reads the immutable Session event snapshot; `appendWal()` and `appendReference()` call `Session.append()` and resolve only after `SessionStore.flush()` confirms a persistence listener completed successfully. The final reference is idempotent for the same thread id and rejects a conflicting id; unknown fields and future versions are rejected before append. It never writes Session header metadata and never mirrors Codex storage.

JSONL and SQLite can consume this same DSH Session seam one mount at a time: the Session owner selects one `SessionPersistence` implementation, and the journal uses only the common `SessionStore.flush()` contract. It does not dual-write or make both physical backends co-author one log. This is a production persistence seam, not a new provider or a UI capability.

[`CodexStatefulExecution`](src/stateful-execution.ts) is the explicit lower-level turn adapter. Its Session owner durably writes `prepared` and then `accepted(threadId)` `codex/thread-start-wal` records, followed by the one `codex/thread-reference`, before a turn starts. If accepted durability fails after Codex created a thread, only `prepared` remains durable and recovery fails closed with a reconciliation requirement; the adapter does not create a second thread. If the final reference write fails after `accepted` is durable, recovery returns the same thread id, the journal writes the one missing reference, and the adapter resumes that thread. A final reference makes later executions resume only; conflicting, duplicate, unknown-field, and future-version state fails closed.

Codex 0.147.0 has neither a caller idempotency key nor a compensating thread deletion method. The protocol therefore provides at-least-once handling for observed ids, not exactly-once thread creation. Stateful execution coordinates completed turn, local abort, child settlement, JSON-RPC input close/error, transport failure, and malformed terminal notifications; every outcome closes the wire and waits for whole-process-tree quiescence. Public failures use fixed safe messages and do not expose raw stderr, paths, commands, credentials, or upstream error text. UI, item streams, usage, diff, review, and approval bridging remain outside it.

The existing Profile provider does not consume this stateful seam. `apply()` registers only the existing one-shot provider; importing or constructing the stateful classes registers no provider and starts no process. Its one-shot behavior, temporary-thread ownership, unattended approvals, and disabled-by-default product path remain unchanged, as do native DeepSeek Session, tool, attachment, and recovery assembly.
...
The one-shot Provider wire intentionally implements only the app-server methods required by that contract; the explicit stateful adapter adds only the stable `thread/start`, `thread/resume`, `turn/start`, and terminal-notification usage described above. The runtime dependency and all six optional-dependency aliases are pinned to `@openai/codex@0.147.0` / `codex-cli 0.147.0`. A normal install selects one payload for the current OS and CPU. For the current darwin-arm64 payload, `npm pack --dry-run --json @openai/codex@0.147.0-darwin-arm64` reports 111,199,052 packed bytes and 274,777,843 unpacked bytes. That package contains native `codex`, `codex-code-mode-host`, `rg`, and `zsh` resources; other platforms may differ, and these values are disclosure rather than an installation threshold.

The shared package's stable-schema fingerprint and these provider tests pin all sixteen error-info variants, HTTP-status locations, six lifecycle stages, process outcomes, stop-reason mapping, unknown fallback, sanitization, permission ordering, cancellation, concurrency, and cleanup aggregation. The keyless real-product test drives the package wrapper against a loopback Responses fixture and observes the package-local argv, exact Bearer key, original task, byte-exact final answer, thread-level `never` overriding ambient `on-request`, automatic-review startup, unattended rejection without file side effects, a real `internalServerError`, explicit dangerous-bypass writing in suite-owned temporary storage, process/protocol failure with safe exit facts, and wrapper/native quiescence. The same tier proves two named instances retain separate environments and native modes. A separate real 0.147.0 restart test uses a suite-owned CODEX_HOME and DSH JSONL Session across two app-server processes, verifies one persisted reference and one resumed upstream thread id, and waits for both process trees to stop.
...
- **Stateful continuation is explicit and lower-level** — it is not registered as a Profile provider, does not start a process on import/construction, and does not add UI, usage, diff, review, approval, or item-stream surfaces.
- **Stateful recovery is fail-closed** — a durable prepared-only start requires reconciliation; an accepted start can be repaired with its same thread id; a conflicting, duplicate, unknown-field, or future-version reference stops execution.
- **No exactly-once thread creation** — Codex 0.147.0 supplies no caller idempotency key or compensating deletion, so an upstream acceptance followed by a DSH durability failure cannot be made exactly-once by this adapter.
