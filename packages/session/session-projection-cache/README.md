...
Writes for the same live Session reserve their order before awaiting log durability. A delayed creation checkpoint cannot overwrite the later detach checkpoint. Failed writes release their place so a later write can recover, and cache disposal joins pending writes before closing the storage domain.
